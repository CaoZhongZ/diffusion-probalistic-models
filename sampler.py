...
def generate_samples(model, get_mu_sigma,
    ...
    """
    Run the reverse diffusion process (generateive model).
    """
    ...
    XT = rng.normal(size=(n_samples, n_colors, spatial_swidth, spatial_width))
    if denoise_sigma is not None:
    ...
    for t in xrange(model.trajectory_length-1, 0, -1):
        ...
        if np.mod(model.trajectory_length-t,
                  int(np.ceil(model.trajectory_length/(num_intermediate_plot+2.)))) == 0:
